...
def get_mtcars_server_functions(input, output, session):
    ...
    reactive_stock = reactive.Value("Tesla")
    ...
    def mtcars_stock_string():
        logger.info("mtcars_stock_company_string starting")
    ...
    def mtcars_stock_chart():
        df = get_mtcars_price_df()
        df_stock = df[df["Company"] == reactive_stock.get()]
        logger.info(f"Rendering Stock chart with {len(df_stock)} bars")
        plotly_express_plot = px.bar(
            df_stock, x='TIME.index', y="Price.columns", color="Company"
        )
